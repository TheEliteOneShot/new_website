...
export function createApi() {
  ...
  api.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (config.ECHO_ALL_API_ERRORS) logging.debug(`API ERROR: ${JSON.stringify(error)}`)
      ...
      if (status === 401) {
        if (errorMsg === 'REFRESH_TOKEN_INVALID') {
          ...
          if (userSession.isTokenRefreshing) {
            return isTokenRefreshFinished(userSession, 100).then(() => api.request(error.config))
          } else {
